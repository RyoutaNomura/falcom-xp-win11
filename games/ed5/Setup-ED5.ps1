...
<#
================================================================================
 Setup-ED5.ps1  —  英雄伝説V「海の檻歌」Windows版 セットアップ
================================================================================

 構成:
   cnc-ddraw (ddraw.dll を置くだけ) + 互換性レイヤー HIGHDPIAWARE
   + ludusavi wrap によるセーブ同期 + Playnite から起動

 常駐プロセスなし。管理者権限なし。互換モードなし。ランチャーなし。

 このスクリプトが「なぜそうしているか」は docs/ にある。ここには書かない。
   docs/analysis.md     ED5_XP.EXE を解析して確認した事実
   docs/setup.md        必要な作業・設定 / セーブ同期 / 症状別の確認手順
   docs/conventions.md  スクリプトを触るときの規約

 ED3 / ED4 との違い（docs/analysis.md 1.4）:
   - ED3 と同じく exe 自身が DDRAW.dll を import する。DLLDV\ は無い
   - 描画は 640x480 の 8bit (256色) 排他フルスクリーン固定。
     ゲーム側にウィンドウモードの設定は無い → 16BITCOLOR は不要
   - 設定は ED4 と同じくレジストリ。ただし Direct3D / ScreenMode の値は無い
   - セーブは %APPDATA%\FALCOM\ED5_XP 直下（SaveData のようなサブフォルダ無し）
 ...
 使い方:
   .\Setup-ED5.ps1 -VerifyOnly    現状が意図どおりか確認するだけ（変更しない）
   .\Setup-ED5.ps1 -WhatIfOnly    やろうとしている事だけ表示（変更しない）
   .\Setup-ED5.ps1                実行
================================================================================
#>
...
param(
    ...
    # 互換性レイヤー。ED5 は 8bit カラーを要求するので 16BITCOLOR は当てない
    # （16bit を前提にしているのは ED4 の描画ドライバの話）。
    ...
    # ゲーム設定（レジストリ）を一切書き換えない
    [switch] $SkipGameSettings,

    # サーフェイスの置き場所 (HKCU\SOFTWARE\FALCOM\<exe>\Surface)
    #   keep  = 触らない（既定。動いている値を壊さない）
    #   video = 1 … ED5_CFG.EXE の「VRAM: ビデオ」
    #   main  = 0 … ED5_CFG.EXE の「VRAM: メイン」。描画が崩れるときに試す
    [ValidateSet('keep','video','main')]
    [string] $GameSurface = 'keep',
...
$GameId      = 'ed5'
$NativeRes   = '640x480 = 4:3 / 8bit (256色)'
...
$ExeBase  = [System.IO.Path]::GetFileNameWithoutExtension($ExeName)   # ED5_XP
...
# ED5 は ED3 と同じく exe 自身が DDRAW.dll を import する。
# ED4 のような DLLDV\ の描画ドライバは無いので、その確認も無い。
$cfgExe = Join-Path $GameDir 'ED5_CFG.EXE'
if (Test-Path -LiteralPath $cfgExe) {
    Write-Ok "環境設定ツール: $cfgExe"
} else {
    Write-Warn2 'ED5_CFG.EXE がありません。ゲーム設定を GUI から変更できません。'
}

foreach ($d in @('WAVEDVD','MIDI_GM')) {
    $p = Join-Path $GameDir $d
    if (-not (Test-Path -LiteralPath $p)) {
        Write-Warn2 "$d フォルダがありません。インストールが不完全かもしれません。"
    }
}

Write-Info "描画解像度は $NativeRes 前提。表示比率は $AspectRatio で固定する。"
Write-Info 'Windows 11 のデスクトップに 8bit モードは無い。cnc-ddraw が事実上の必須条件。'
...
elseif ($VerifyOnly) {
    ...
    elseif (-not $NoChange) {
        Write-Warn2 'cnc-ddraw を配置できませんでした。ED5 は 8bit モードを要求するので、この状態では起動しない可能性が高いです。'
    }
}
...
if ($layerValue -match '16BITCOLOR') {
    Write-Info '16BITCOLOR が入っていますが、ED5 には不要です（16bit 前提なのは ED4 の描画ドライバ）。'
    Write-Info 'ED5 が要求するのは 8bit で、それは cnc-ddraw が受けています。'
}
...
# ED5_CFG.EXE のテーブルから読んだ値名（docs/analysis.md 1.4）
$known = @(
    'BGM','BgmVolume','SEVolume','SE','Joystick',
    'DirectInput','DirectSound','Flip','Surface','Frame'
)
...
if (Test-Path -LiteralPath $gameKey) {
    ...
    Write-Info '(まだ存在しません。ED5_CFG.EXE を一度実行すると作られます)'
}
...
if ($SkipGameSettings) {
    ...
    # ED4 と違い、ED5 には ScreenMode も EnableDirect3D も無い。
    #   - 画面モードは exe 側で排他フルスクリーン固定（ゲーム側に選択肢が無い）
    #   - Direct3D を使う経路が無いので切る設定も無い
    # したがって既定では何も書き込まない。-GameSurface を明示したときだけ書く。
    Write-Info 'ED5 には ScreenMode / Direct3D の設定がありません（exe 側で排他フルスクリーン固定）。'

    if ($GameSurface -eq 'keep') {
        $sf = $null
        if (Test-Path -LiteralPath $gameKey) {
            $g0 = Get-ItemProperty -LiteralPath $gameKey -ErrorAction SilentlyContinue
            if ($g0 -and $g0.PSObject.Properties.Name -contains 'Surface') { $sf = $g0.Surface }
        }
        if ($null -eq $sf) {
            Write-Info 'Surface は未設定です（ED5_CFG.EXE の既定は「ビデオ」）。触りません。'
        } else {
            $label = if ($sf -eq 0) { 'メイン' } else { 'ビデオ' }
            Write-Ok "Surface = $sf ($label) のまま触りません"
        }
    }
    else {
        $want  = if ($GameSurface -eq 'main') { 0 } else { 1 }
        $label = if ($want -eq 0) { 'メイン' } else { 'ビデオ' }
        $now   = $null
        if (Test-Path -LiteralPath $gameKey) {
            $g = Get-ItemProperty -LiteralPath $gameKey -ErrorAction SilentlyContinue
            if ($g -and $g.PSObject.Properties.Name -contains 'Surface') { $now = $g.Surface }
        }
        if ($now -eq $want) {
            Write-Ok "Surface = $want ($label) （既に設定済み）"
        }
        elseif ($VerifyOnly) {
            Write-Warn2 "Surface が $want ($label) ではなく $now です。"
        }
        else {
            if (Test-Change "Surface を $now から $want ($label) に変更") {
                if (-not (Test-Path -LiteralPath $gameKey)) { New-Item -Path $gameKey -Force | Out-Null }
                New-ItemProperty -LiteralPath $gameKey -Name 'Surface' -Value $want -PropertyType DWord -Force | Out-Null
            }
        }
    }

    Write-Info 'Frame / 音量 / BGM デバイスは触りません（端末ごとに好みで）。'
}
...
if (-not $useLudusavi) {
    ...
    Write-Info 'ED5 の設定はレジストリにあるので、ED3 の ED3_CFG.INI のような'
    Write-Info '「同期から外す」指定は不要です。Frame は自動的に端末ごとの値になります。'
    Write-Info 'スクリーンショット (BMP\) はゲームフォルダ側なので同期対象外です。'
...
elseif (Test-Path -LiteralPath $iniPath) {
    ...
    if ($chk -match 'HIGHDPIAWARE' -and $chk -notmatch 'WINXPSP3') {
...
Write-Host @"
  ...
  次にやること
  ------------
  1. ED5_CFG.EXE（環境設定）を開いて確認する
       BGM             : WAVE（WAVEDVD\ の WAV を鳴らす）/ MIDI / なし
       VRAM            : 描画が崩れる場合は「メイン」を試す（-GameSurface main）
       描画速度固定    : 自動 / 同期
       描画精度        : 標準 / 少し粗い / 粗い（進行速度が変わる。docs/setup.md 7.2）
     画面モードの設定は存在しない（exe 側で排他フルスクリーン固定）。
     ウィンドウ表示は cnc-ddraw 側で作る。
  2. $ExeName を直接起動して、$AspectRatio で映ることを確認する
  3. 一度セーブして %APPDATA%\FALCOM\$ExeBase\ に ED5SD*.DAT ができたことを確認する
  ...
  ------------------
    ...
    「DirectDraw Init FAILED」が出るなら ddraw.dll が読まれていない。
